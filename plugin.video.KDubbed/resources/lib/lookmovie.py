...
handlers_flixhq.BASE_URL = "https://lookmovie.tube/"
BASE_URL = handlers_flixhq.BASE_URL


# ── GENRE MENU ────────────────────────────────────────────────
def GENRE_LMENU():
    try:
        # Use /movie page instead of homepage to access sidebar
        soup = OpenSoup_SG(urljoin(BASE_URL, "/movie"))

        genre_section = soup.select_one("#sidebar_subs_genre")
        if not genre_section:
            raise Exception("Genre section not found on /movie page")

        genre_links = genre_section.select("a[href^='/genre/']")
        if not genre_links:
            raise Exception("No genre links found")

        for a in genre_links:
            title = a.get_text(strip=True)
            href = a["href"]
            xbmc.log(f"[{ADDON_ID}] Genre: {title} -> {href}", xbmc.LOGINFO)
            addDir(title, urljoin(BASE_URL, href), "index_lookm")

        xbmcplugin.endOfDirectory(PLUGIN_HANDLE)

    except Exception as e:
        xbmc.log(f"[{ADDON_ID}] GENRE_MENU error: {e}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification("Error", "Failed to load genres", xbmcgui.NOTIFICATION_ERROR)

# ── Country Menu ────────────────────────────────────────────────
def COUNTRY_LMENU():
    try:
        # Use /movie page for sidebar (static HTML)
        soup = OpenSoup_SG(urljoin(BASE_URL, "/movie"))

        country_section = soup.select_one("#sidebar_subs_country")
        if not country_section:
            raise Exception("Country section not found on /movie page")

        country_links = country_section.select("a[href^='/country/']")
        if not country_links:
            raise Exception("No country links found")

        seen = {}

        # Collect all found countries
        for a in country_links:
            href = a["href"]
            title = a.get_text(strip=True)
            seen[href] = title

        # Manually add Philippines if missing
        ...
        for href, title in sorted(seen.items(), key=lambda x: x[1].lower()):
            xbmc.log(f"[{ADDON_ID}] Country: {title} → {href}", xbmc.LOGINFO)
            addDir(title, urljoin(BASE_URL, href), "index_lookm")

        xbmcplugin.endOfDirectory(PLUGIN_HANDLE)

    except Exception as e:
        xbmc.log(f"[{ADDON_ID}] COUNTRY_MENU error: {e}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification("Error", "Failed to load countries", xbmcgui.NOTIFICATION_ERROR)


# ── Listing Handler ─────────────────────────────────────────────
def INDEX_LOOKM(url, label_suffix=""):
    try:
        xbmc.log(f"[{ADDON_ID}] Fetching index: {url}", xbmc.LOGINFO)
        html = OpenURL_SG(url, as_text=True)
        ...
        for item in items:
            ...
            addDir(display_title, "https://lookmovie.tube/"+link, "type", iconimage=image)
        ...
        if current_page > 1:
            addDir("< Previous Page", build_page_url(current_page - 1), "index_lookm")

        addDir("Next Page >", build_page_url(current_page + 1), "index_lookm")

    except Exception as e:
        xbmc.log(f"[{ADDON_ID}] index_lookm error: {e}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification("Error", str(e), xbmcgui.NOTIFICATION_ERROR)

def SEARCH_LOOKM():
    search_text = GetInput("Enter Search Text", "Search LookMovie")
    if not search_text:
        return
    ...
    try:
        ...
        xbmc.log(f"[{ADDON_ID}] Searching LookMovie: {search_url}", xbmc.LOGINFO)

        INDEX_LOOKM(search_url)
        xbmcplugin.endOfDirectory(PLUGIN_HANDLE)

    except Exception as e:
        xbmc.log(f"[{ADDON_ID}] SEARCH_LOOKM error: {e}", xbmc.LOGERROR)
...
def TYPE(url, name):
    ...
    if 'data-type="1"' in html:
        ...
        sid = "https://lookmovie.tube/ajax/episode/list/"+str(vid)
        return PLAYLOOP_SG(sid, name)
    elif 'data-type="2"' in html:
        vid = re.search(r'[0-9].*', url)[0]
        sid = "https://lookmovie.tube/ajax/season/list/"+str(vid)
...
def SEASON(url, name):
    ...
    for link in div_index:
        ...
        addDir(vTitle,'https://lookmovie.tube/ajax/season/episodes/'+vLink, "episode")
...
def EPISODE(url, name):
    ...
    for link in div_index:
        vLink = link("a")[0]["data-id"]
        vTitle = link('a')[0].text
        addDir(vTitle, f"https://lookmovie.tube/ajax/episode/servers/{vLink}", "server")
...
def SERVER(url,name):
    ...
    for link in div_index:
        vLink = link("a")[0]["data-id"]
        vTitle = link('a')[0].text
        addLink(vTitle, f"http://dodge.eu5.org/go/?url=https://lookmovie.tube/ajax/episode/sources/{vLink}", "play_server")
...
def PLAYLOOP_SG(url, name):
    ...
    for link in div_index:
        ...
        addLink(vTitle, "http://dodge.eu5.org/go/?url=https://lookmovie.tube/ajax/episode/sources/"+vLink, "play_server")
